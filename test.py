...
def get_args():
    # Cac tham so test co the truyen tu command line.
...
def test(opt):
    # Co dinh seed de test on dinh hon.
    if torch.cuda.is_available():
        torch.cuda.manual_seed(123)
    else:
        torch.manual_seed(123)

    # Chon tap action giong luc train.
    ...
    # Tao env test va file video output.
    ...
    try:
        ...
        # Chay nhieu attempt den khi qua man hoac het gioi han.
        while episode <= opt.test_episodes:
            if torch.cuda.is_available():
                state = state.cuda()
            logits, value = model(state)
            policy = F.softmax(logits, dim=1)
            # Deterministic dung argmax, mac dinh sample theo policy.
            ...
            if info["flag_get"]:
                ...
                break
            # Reset khi attempt ket thuc hoac vuot max_steps.
        ...
        # Dong env du test thanh cong hay loi.
        env.close()
    if completed:
